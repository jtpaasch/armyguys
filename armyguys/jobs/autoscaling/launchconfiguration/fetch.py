...
"""Jobs for fetching launch configuration data."""
...
def get_all(
    ...
    Args:
        ...
        job_text_reporter
            Job text strings from jobs will be sent to this reporter.
    ...
    if job_heading_reporter:
        job_heading_reporter(reports, "Fetching launch configurations")
    params = {}
...
def get_by_name(
    ...
    """
    if job_heading_reporter:
        job_heading_reporter(reports, "Fetching launch configuration by name")
    params = {}
    params["profile"] = profile
    response = launchconfiguration.get(**params)
    utils.check_response_is_ok(
    ...
    if len(data) > 0:
        for record in data:
            if hasattr(record, "get"):
                if record.get("LaunchConfigurationName") == name:
                    result = record
                    break
    if job_data_reporter:
        job_data_reporter(reports, result)
